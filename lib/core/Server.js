...
exports.Server = SC.Object.extend({
  ...
  createHTTPHandler: function(serverObj){
    ...
    var handlerFunctions = [ 'handleRoot', 'handleAUTHRequest','handleSession','handleRPCGET','handleUploadPOST'];
...
// ===========================   
  ...
  RPC: function(request,resource,response){
   ...
   UPLOAD: function(request,resource,response){
      // UPLOAD accepts a JSON object { uploadRequest: { cacheKey: ''}}
      // call the UPLOAD
      // split the cacheKey from the resource
             ...
             this.tempStore.refreshDBRecord(this._createTempURLStoreRequest(bucket,key),function(rec){
               if(rec){ // something found, so parse it
                 if(me._checkTempURLValidity.call(me,bucket,key,rec)){ // function auto-updates tempStore
                   ...
                   this.uploadModule.performUpload(request, cb);
                   return;
                 }
               }
             });
         ...
         if(uploadRequest){
            ...
            this.uploadModule.callUploadFunction(fn,params,function(data){      // callback will register tmp url, if permission granted
               ...
               if (permission === 'granted') {
                 ...
                 me.registerTemporaryURL('upload', data.uploadRequestResult.cacheKey, 'application/json', null, 0, 'num', false);
               }
