...
exports.Server = SC.Object.extend({
   ...
   init: function(){
     ...
     this.UploadURL = '/upload'; // setting a default value when an instance is made
   ...
   _attachSocketListener: function(){
      ...
      this.socketIO.start(this.server,{
        ...
        onClientMessage: function(message, client){
           ...
           sys.log('onClientMessage in ThothServer called with message: ' + sys.inspect(message));
  ...
  createHTTPHandler: function(serverObj){
    ...
    var handlerFunctions = [ 'handleRoot', 'handleAUTHRequest','handleRPCGET','handleUploadPOST','handleSession'];
    
    var handlers = {
      handleRoot: function(path,method,request,response){
        sys.log('ROOT THIS IS THE PATH ' + path);
        if(path === '/'){
          if(serverObj.debug){
      ...
      handleAUTHRequest: function(path,method,request,response){
        sys.log('AUTH THIS IS THE PATH ' + path);
      ...
      handleSession: function(path,method,request,response){
        sys.log('SESSION THIS IS THE PATH ' + path);
      ...
      handleRPCGET: function(path,method,request,response){
        sys.log('RPC THIS IS THE PATH ' + path);
        var resource;
        if(method === 'GET'){
      ...
      handleUploadPOST: function(path,method,request,response){
        var resource;
        sys.log('THIS IS THE PATH ' + path);
        sys.log('THIS IS THE METHOD ' + method);
        if(method === 'POST'){
          resource = path.slice(1);
          sys.log('THIS IS THE RESOURCE ' + resource);
...
// ===========================   
  ...
  RPC: function(request,resource,response){
   ...
   UPLOAD: function(request,resource,response){
      ...
      var resourceInfo = resource.split('/');
      sys.log('resourceInfo ' + resourceInfo);
      if(resourceInfo && (resourceInfo.length===2)){
         var cacheKey = resourceInfo[1];
         if(cacheKey && this.uploadModule){
           ...
           if(parts.length === 2){ // if not, someone is playing with us... and we should ignore it
             ...
             sys.log('key |' + key + '|');
             var me = this;
             this.tempStore.refreshDBRecord(this._createTempUploadURLStoreRequest(bucket,key),null,function(rec){
               sys.log('^^^^^^^^^^^^^^^^^ rec ' + rec);
                 ...
                 if(me._checkTempURLValidity.call(me,bucket,key,rec)){ // function auto-updates tempStore
                   ...
                   var cb = function(data){
                     if(data){
                       ...
                       response.write('upload complete'); // TODO: what goes in response?
                       //response.write(data,'binary');
                       response.end();
                     } else {
                   ...
                   sys.log('calling upload................ ');
                   me.uploadModule.performUpload(request, cb);
                   return;
                 }
               }
             });
...
// ==========================================
  ...
  registerTemporaryUploadURL: function(bucket,key,data,mimeType,limit,limitType,shouldAnonymize){
    if(!this.tempStore){
      sys.log('Thoth Server: unable to register temporary url because there is no temporary store??');
      return false;
    }
    // the temporary url will be stored in the temp store with tempURL as bucket and the expected resource as key
    sys.log('storing......... bucket |' + bucket + '|');
    sys.log('storing......... key |' + key + '|');
    var record = {
      data: data,
      mimeType: mimeType,
      limit:limit,
    ...
    this.tempStore.createDBRecord(this._createTempUploadURLStoreRequest(bucket,key,record));
  },
  ...
  getTemporaryURL: function(request,response,callback){
    ...
    if(parts.length === 2){ // if not, someone is playing with us... and we should ignore it
      ...
      this.tempStore.refreshDBRecord(this._createTempURLStoreRequest(bucket,key),null,function(rec){
        if(rec){ // something found, so parse it
          if(me._checkTempURLValidity.call(me,bucket,key,rec)){ // function auto-updates tempStore
  ...
  _createTempUploadURLStoreRequest: function(bucket,key,record){
    var storeRequest = {
      bucket: bucket,
      key: key,
      recordData: record
...
// make sure you use these functions when you are writing a REST interface that should interact with the socket clients!  
 ...
 /*  
   ...
   */
   ...
   onUploadRequest: function(message,userData,callback){
      var me = this;
      ...
      if(this.uploadModule){
         ...
         if(uploadRequest){
            ...
            this.uploadModule.callUploadFunction(fn,params,function(data){      // callback will register tmp url, if permission granted
               ...
               if (permission === 'granted') {
                 ...
                 data.uploadRequestResult.uploadURL = '/upload/' + data.uploadRequestResult.cacheKey;
                 me.registerTemporaryUploadURL(
                   'upload',
                   data.uploadRequestResult.cacheKey,
                   { thumbnails: { small: { w: 32, h: 32 }, medium: { w: 128, h: 128 }, large: { w: 256, h: 256 }}},
                   'application/json',
                   0,
                   'num',
                   false
               ...
               callback(data); // the data contains data.uploadRequestResult.uploadURL if permission granted
            });
         }
      }
   }
