   ...
   onUploadRequest: function(message,userData,callback){
      ...
      if(this.uploadModule){
         ...
         if(uploadRequest){
            ...
            this.uploadModule.callUploadFunction(fn,params,function(data){
               data.uploadResult.returnData = uploadRequest.returnData;
               sys.log('permission ', data.permission);
               if (data.permission === 'granted') {
                 sys.log('permission granted ', data.uploadResult.cacheKey);
                 data.uploadResult.uploadURL = 'upload/' + data.uploadResult.cacheKey;
                 this.registerTemporaryURL('upload', data.uploadResult.cacheKey, 'application/json', null, 0, 'num', false);
               }
               callback(data); // the data contains data.uploadResult.uploadURL if permission granted
            });
         }
      }
   }
