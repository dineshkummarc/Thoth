      ...
      return function(request, response){
         ...
         if(path === '/'){
           if(this.debug){
            ...
            switch(method){
               case 'UPLOAD':
                  serverObj.UPLOAD(request,postdata,response);
                  break;
               case 'GET':
...
// ===========================   
   ...
   UPLOAD: function(request,resource,response){
      // UPLOAD accepts a JSON object { rpcRequest: { cacheKey: ''}}
      // do the UPLOAD
      // split the cacheKey from the resource
      ...
      if(resourceInfo && (resourceInfo.length===2)){
         ...
         if(cacheKey && this.uploadModule){
            ...
            var cb = function(mimeType,data){
               if(mimeType && data){
                  response.writeHead(200, {'Content-Type': mimeType });
                  response.write("<br/>upload result: " + data);
                  response.end();
               }
               else me.send404(response); // force the end of the response if nothing can be found?              
            };
            sys.log('ThothServer: about to call uploadRetrieve on uploadModule');
            this.uploadModule.uploadRetrieve(cacheKey, cb);
         }
      }
   },
